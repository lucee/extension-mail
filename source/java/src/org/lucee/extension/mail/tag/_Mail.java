...
public abstract class _Mail extends TagImpl {
	...
	public void setAttachmentpath(String attachmentPath) throws PageException {
		...
		if (!attachmentDir.exists() && !attachmentDir.mkdir()) {
			...
			if (!attachmentDir.exists() && !attachmentDir.mkdir())
				throw CFMLEngineFactory.getInstance().getExceptionUtil().createAbortException(
						"Directory [" + attachmentPath + "] doesn't exist and couldn't be created");
		}
		if (!attachmentDir.isDirectory())
			throw CFMLEngineFactory.getInstance().getExceptionUtil()
					.createAbortException("File [" + attachmentPath + "] is not a directory");
	...
	public int doStartTag() throws PageException {
		...
		if (!Util.isEmpty(delimiter) && uid == null)
			throw CFMLEngineFactory.getInstance().getExceptionUtil()
					.createAbortException("must specify the attribute [uid] when the attribute delimiter is defined");
		...
		try {
			...
			id = engine.getCastUtil().toString(bif.invoke(pageContext, new Object[] {}));
		} catch (Exception e) {
		...
		if (!Util.isEmpty(connection) && Util.isEmpty(server)) {

		}

		client.setDelimiter(!Util.isEmpty(delimiter, true) ? delimiter : ",");
		client.setTimeout(timeout * 1000);
		client.setMaxrows(maxrows);
		if (startrow > 1)
			client.setStartrow(startrow - 1);
		client.setUniqueFilenames(generateUniqueFilenames);
		if (attachmentPath != null)
			client.setAttachmentDirectory(attachmentPath);
		...
		try {
			...
			// imap only
				...
				if (getType() == MailClient.TYPE_IMAP)
					actions += ",open,close,markread,createfolder,deletefolder,renamefolder,listallfolders,movemail";

				throw CFMLEngineFactory.getInstance().getExceptionUtil().createAbortException(
						"Invalid value for attribute [action], valid values are [" + actions + "]");
			}
		} catch (Exception e) {
			throw CFMLEngineFactory.getInstance().getCastUtil().toPageException(e);
		} finally {
	...
	private void checkConnection() throws PageException {
		if (Util.isEmpty(connection) && Util.isEmpty(server)) {
			throw CFMLEngineFactory.getInstance().getExceptionUtil()
					.createAbortException("You need to define the attribute [connection] or [server].");
		}
	}
