...
public final class SMTPConnectionPool {
	...
	public static SessionAndTransport getSessionAndTransport(Properties props, String key, Authenticator auth,
			long lifeTimespan, long idleTimespan) throws MessagingException {
	...
	private static Stack<SessionAndTransport> getSATStack(String key) {
		...
		if (stack == null) {
			synchronized (MailUtil.createToken("getSATStack", key)) {
				stack = sessions.get(key);
				if (stack == null) {
	...
	public static class SessionAndTransport {
		...
		SessionAndTransport(String key, Properties props, Authenticator auth, long lifeTimespan, long idleTimespan)
