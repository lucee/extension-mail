...
public final class ServerImpl implements Server, Serializable {
	...
	public static ServerImpl getInstance(String host, int defaultPort, String defaultUsername, String defaultPassword,
			long defaultLifeTimespan, long defaultIdleTimespan, boolean defaultTls, boolean defaultSsl)
		...
		if (index != -1) {
			...
			if (!Util.isEmpty(tmp)) {
				try {
					port = CFMLEngineFactory.getInstance().getCastUtil().toIntValue(tmp);
		...
		return new ServerImpl(-1, host, port, user, pass, defaultLifeTimespan, defaultIdleTimespan, defaultTls,
				defaultSsl, true, TYPE_LOCAL);
	}

	/*
	 * public ServerImpl(String server,int port) { this.hostName=server;
	 * this.port=port; }
	 */
	...
	public String getPassword() {
		if (password == null && hasAuthentication())
			return "";
		return password;
	}
	...
	public static lucee.runtime.net.mail.Server[] merge(lucee.runtime.net.mail.Server[] arr1,
			lucee.runtime.net.mail.Server[] arr2) {
